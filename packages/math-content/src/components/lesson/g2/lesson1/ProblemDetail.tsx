...
import { TAG_STYLE, TYPE_TIP } from '@rosie/math-content/utils/g2/lesson1-data'
...
export default function ProblemDetail({
  ...
  const { practiceCount, correctCount, handleSolve, addWrong } = useG2Lesson1()
  ...
  const { answer, setAnswer, feedback, submit, check, clearFeedback, hasAttempted } = useProblemAnswer(
    ...
    {
      wrongHint: '❌ 不对哦，再想想？提示：可以用凑整、去括号、基准数等方法巧算。',
    },
  )
  ...
  const tipBlock = tipText ? (
    <div className="mt-3 rounded-lg bg-indigo-50 px-3 py-2.5 text-xs leading-relaxed text-indigo-800">
      💡 <strong>巧算口诀：</strong>
      {tipText}
    </div>
  ) : null
  ...
  const answerDom = (
    <ProblemAnswerSection
      ...
      buttonClassName="bg-indigo-600 shadow-[0_3px_10px_rgba(67,56,202,0.3)]"
      tip={tipBlock}
    />
  )
