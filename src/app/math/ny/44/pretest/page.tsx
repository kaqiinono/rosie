...
import ProblemList from '@/components/math/lesson44/ProblemList'
  ...
  return (
    <div>
      <div className="mb-3.5 rounded-[14px] border border-yellow-200 bg-gradient-to-br from-yellow-50 to-[#fef9c3] p-4">
        <div className="mb-1 text-sm font-extrabold text-yellow-900">📝 课前测 · 第44讲</div>
        <div className="mb-2 text-xs text-yellow-700">{total}道摸底题 · 统筹六大题型</div>
        <div className="flex items-center gap-2">
          <div className="relative h-1.5 flex-1 overflow-hidden rounded-full bg-yellow-100">
            <div
              className="absolute inset-y-0 left-0 rounded-full bg-yellow-200 transition-[width] duration-400"
              style={{ width: `${total > 0 ? Math.round((attempted / total) * 100) : 0}%` }}
            />
            <div
              className="absolute inset-y-0 left-0 rounded-full bg-yellow-500 transition-[width] duration-400"
      ...
      <ProblemList problems={list} solveCount={solveCount} basePath="/math/ny/44/pretest" />
    </div>
  )
}
