    ...
    blocks: [
      {
        ...
        body: [
          ...
          '「限时答题」控制是否编辑各题型目标时间；关掉后仍用系统目标时间；宽松模式显示软倒计时（超时标红、不强制交卷）。',
          '「答对即过」（默认开）：数字键盘或竖式填满正确答案自动提交，答错仍须确认。',
          '设置里可选默认计时模式（宽松 / 严格 / 自定义加成），开始练习前会预填、可当场改。',
        ],
      },
    ],
  },
    ...
    blocks: [
      {
        ...
        body: [
          ...
          '对了得星星；错了通常可再试一次，还不对会进错题，本场末尾还会补练（有上限，见下）。',
          '上一场还没练会的错题，可能混在本场里再出现。',
          '结束后可看正确率与用时；严格/加成模式有场次星星加成，星星可去换奖券。',
        ],
      },
    ],
  },
    ...
    blocks: [
      {
        title: '1. 新题优先（九九表一类）',
        body: [
          '对 2～9 的乘除以及 100 以内凑整，会尽量先出还没练过的算式，少反复刷已经见过的。',
    ...
    blocks: [
      {
        title: '三种计时模式',
        body: [
          '宽松：显示目标软倒计时，超时标红但不强制交卷，星星无加成（×1.0）。',
          '严格：倒计时到 0 算最终错误，星星 +20%（×1.2）。',
          '自定义加成：倒计时 = 目标时间 + 加成秒数；星星加成随加成递减（每多 1 秒少 5 个百分点，最低 ×1.0）。',
          '自定义模式下倒计时含加成，但「算得顺不顺」仍按目标时间判断——在两者之间答对可能仍记卡顿。',
        ],
      },
      {
        title: '补练上限与单次清算',
        body: [
          '主路径约 N 题；补练池上限 M = max(3, floor(N×15%))。',
          '主路径做错或严格/加成超时 → 尽量入补练池；池满则本场不再补练该题。',
          '补练每题只一次机会，做完即结束，不会无限排队。',
        ],
      },
    ],
  },
    ...
    blocks: [
      {
        title: '必须开「限时答题」吗？',
        body: [
          '不必。关掉只是不编辑各题型目标时间；每场开始前仍可选严格/加成模式。',
          '宽松模式始终按目标时间显示软倒计时；超时时钟标红，仍可继续作答，系统按目标时间判断快慢。',
      ...
      },
      {
        title: '三种计时模式有什么区别？',
        body: [
          '见「计时与补练」：宽松不强制交卷；严格到点交卷、星星 +20%；自定义加成多给缓冲秒数、星星加成递减。',
        ],
      },
      {
        title: '补练会无限做下去吗？',
        body: [
          '不会。每场补练最多 max(3, 总题数×15%) 道，每道只一次机会。',
        ],
      },
      {
        title: '为什么老出同一类题？',
    ...
    blocks: [
      {
        title: '怎么安排',
        body: [
          '入门：少勾几种题型，题量约 20；先求正确，再设目标时间。',
          '巩固九九表：勾 2～9 乘除相关，让系统帮着先练没见过的。',
          '提速：设目标时间，选严格或适当加成；关注「对但慢」是否变少。',
