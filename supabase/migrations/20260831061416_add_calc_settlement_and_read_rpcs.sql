...
BEGIN
  ...
  LOOP
    ...
    -- Compatibility projection during the dual-read window. It is updated in
    -- the same transaction, but question_log + problem_state remain authoritative.
    IF COALESCE((state_item->>'needs_remediation')::boolean, false) THEN
      SELECT entry INTO log_item
      FROM jsonb_array_elements(question_log) WITH ORDINALITY AS logged(entry, ordinal)
      WHERE entry->>'signature' = state_item->>'signature'
        AND NOT COALESCE((entry->>'finallyOk')::boolean, false)
      ORDER BY ordinal DESC LIMIT 1;
      IF log_item IS NOT NULL THEN
        INSERT INTO public.calc_mistakes (
          user_id, signature, display, answer, answer_json, level, category,
          last_wrong_at, consecutive_correct, resolved, session_no, user_answer, error_tag
        ) VALUES (
          owner_id, state_item->>'signature', COALESCE(log_item->>'display', state_item->>'signature'),
          CASE log_item->'answerJson'->>'kind'
            WHEN 'int' THEN (log_item->'answerJson'->>'value')::integer
            WHEN 'decimal' THEN round((log_item->'answerJson'->>'value')::numeric)::integer
            WHEN 'remainder' THEN (log_item->'answerJson'->>'quotient')::integer
            WHEN 'fraction' THEN (log_item->'answerJson'->>'num')::integer
            ELSE 0 END,
          log_item->'answerJson', state_item->>'level',
          CASE
            WHEN log_item->>'key' LIKE 'mixed:%' THEN 'mixed'
            WHEN COALESCE(log_item->>'sourceBlockId', '') LIKE 'mul:%'
              OR COALESCE(log_item->>'sourceBlockId', '') LIKE 'div:%' THEN 'muldiv'
            ELSE 'addsub' END,
          COALESCE(NULLIF(state_item->>'last_wrong_at', '')::timestamptz, now()),
          COALESCE((state_item->>'remediation_correct_count')::smallint, 0), false,
          next_session_no, NULLIF(log_item->>'finalAnswer', ''),
          NULLIF(log_item->>'errorTag', '')
        ) ON CONFLICT (user_id, signature) DO UPDATE SET
          display = EXCLUDED.display, answer = EXCLUDED.answer,
          answer_json = EXCLUDED.answer_json, level = EXCLUDED.level,
          category = EXCLUDED.category, last_wrong_at = EXCLUDED.last_wrong_at,
          consecutive_correct = EXCLUDED.consecutive_correct, resolved = false,
          session_no = EXCLUDED.session_no, user_answer = EXCLUDED.user_answer,
          error_tag = EXCLUDED.error_tag;
      END IF;
    ELSE
      UPDATE public.calc_mistakes SET
        consecutive_correct = GREATEST(
          consecutive_correct,
          COALESCE((state_item->>'remediation_correct_count')::smallint, 0)
        ),
        resolved = true,
        session_no = next_session_no
      WHERE user_id = owner_id AND signature = state_item->>'signature' AND NOT resolved;
    END IF;
  END LOOP;
  ...
  LOOP
    ...
    WHERE user_id = owner_id AND block_id = target_block
      AND curriculum_version = target_version AND applied_revision <= expected_revision;
    IF NOT FOUND THEN
  ...
  SELECT jsonb_build_object(
    ...
    'blocks', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'recentIndependentCorrect', recent_independent_correct,
        'recentIndependentTotal', recent_independent_total, 'stableCount', stable_count,
        'tier', tier, 'ready', ready, 'recovery', recovery,
        'appliedRevision', applied_revision, 'healthStatus', health_status
      ) ORDER BY block_id)
      FROM public.calc_block_progress WHERE user_id = auth.uid()
    ), '[]'::jsonb)
  );
$$;
...
BEGIN
  ...
  FROM (
    ...
    WHERE user_id = auth.uid() AND block_id = p_request->>'blockId'
