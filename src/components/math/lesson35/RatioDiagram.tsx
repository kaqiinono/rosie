    ...
    <svg className="mx-auto block w-full max-w-[520px]" viewBox="0 0 340 320" xmlns="http://www.w3.org/2000/svg">
      ...
      {problem.rows.map((r, i) => (
        <g key={`row-${i}`}>
          ...
          <text x="85" y={ly[i] + 22} textAnchor="middle" dominantBaseline="middle" fontSize="13" fill="#1d4ed8" fontWeight="600">{r}</text>
        </g>
      ))}
      ...
      {problem.rcols.map((r, i) => (
        <g key={`rcol-${i}`}>
          ...
          {typeof r === 'string' ? (
            <text x="255" y={ly[i] + 22} textAnchor="middle" dominantBaseline="middle" fontSize="13" fill="#15803d" fontWeight="600">{r}</text>
          ) : (
...
function Ratio3ColSVG({ problem, values, onChange, inputClass }: SvgProps) {
  ...
  return (
    <svg className="mx-auto block w-full max-w-[520px]" viewBox="0 0 470 320" xmlns="http://www.w3.org/2000/svg">
      <defs>
      ...
      {/* H dashes A↔B and B↔C (3 rows) */}
      {[40, 160, 280].map(y => (
        <g key={`dash-${y}`}>
          <line x1="127" y1={y} x2="173" y2={y} className="stroke-[#cbd5e1] stroke-[1.5]" strokeDasharray="4 3" />
          <line x1="287" y1={y} x2="333" y2={y} className="stroke-[#cbd5e1] stroke-[1.5]" strokeDasharray="4 3" />
        </g>
      ))}

      {/* V arrows - col A blue, col B indigo, col C green */}
      {[
        ...
        <g key={`varr-${cx}`}>
          <line x1={cx} y1={70} x2={cx} y2={129} stroke={color} strokeWidth="1.8" markerEnd={`url(#${mk})`} />
          <line x1={cx} y1={192} x2={cx} y2={249} stroke={color} strokeWidth="1.8" markerEnd={`url(#${mk})`} />
        </g>
      ))}
      ...
      {problem.rows.map((r, i) => (
        <g key={`rA-${i}`}>
          ...
          <text x={cA} y={ly[i] + 22} textAnchor="middle" dominantBaseline="middle" fontSize="12" fill="#1d4ed8" fontWeight="600">{r}</text>
        </g>
      ))}
      ...
      {rows2.map((r, i) => (
        <g key={`rB-${i}`}>
          ...
          {typeof r === 'string' ? (
            <text x={cB} y={ly[i] + 22} textAnchor="middle" dominantBaseline="middle" fontSize="12" fill="#4338ca" fontWeight="600">{r}</text>
          ) : (
            <foreignObject x="177" y={ly[i] + 2} width="106" height="40">
              <div className="flex h-full items-center justify-center gap-0.5">
                <input
                  ...
                  onChange={e => onChange((r as RatioInput).id, e.target.value)}
                  style={{ borderBottom: '1.5px solid', borderBottomColor: inputClass((r as RatioInput).id, true) === 'correct' ? '#22c55e' : inputClass((r as RatioInput).id, true) === 'incorrect' ? '#ef4444' : '#9ca3af' }}
                />
              </div>
            </foreignObject>
          )}
      ...
      {problem.rcols.map((r, i) => (
        <g key={`rC-${i}`}>
          ...
          {typeof r === 'string' ? (
            <text x={cC} y={ly[i] + 22} textAnchor="middle" dominantBaseline="middle" fontSize="12" fill="#15803d" fontWeight="600">{r}</text>
          ) : (
      ...
      {/* Col A circles (blue) */}
      {[
        { cx: cA + 22, cy: 100, o: ops[0], fill: '#3b82f622', stroke: '#93c5fd', color: '#3b82f6' },
        { cx: cA + 22, cy: 220, o: ops[1], fill: '#1d4ed822', stroke: '#3b82f6', color: '#1d4ed8' },
      ].filter(c => c.o).map(({ cx, cy, o, fill, stroke, color }) => (
        <g key={o!.id}>
      ...
      {[
        { cx: cB + 22, cy: 100, o: ops[2], fill: '#6366f122', stroke: '#a5b4fc', color: '#6366f1' },
        { cx: cB + 22, cy: 220, o: ops[3], fill: '#4338ca22', stroke: '#6366f1', color: '#4338ca' },
      ].filter(c => c.o).map(({ cx, cy, o, fill, stroke, color }) => (
        <g key={o!.id}>
          <circle cx={cx} cy={cy} r="14" fill={fill} />
          <foreignObject x={cx - 13} y={cy - 13} width="26" height="26">
