...
import { TAG_STYLE } from '@rosie/math-content/utils/g1/lesson23-data'
...
export default function ProblemDetail({
  ...
  const { practiceCount, correctCount, handleSolve, addWrong } = useG1Lesson23()
  ...
  const solution = <ProblemSolutionPanel problem={problem} variant="violet" heading="推理过程" />
  ...
  const question = (
    <div className="flex flex-col gap-1.5">
      <div className="min-w-0 flex-1">
        ...
        <div
          className="text-text-secondary [&>strong]:text-text-primary mb-3.5 rounded-lg border-l-3 border-violet-300 bg-violet-50 px-3.5 py-3 text-sm leading-relaxed [&>strong]:font-bold"
  ...
  const answerDom = (
    <NumericAnswerPanel
      ...
      onCheck={check}
      feedback={feedback}
      buttonClassName="bg-violet-600 shadow-[0_3px_10px_rgba(124,58,237,0.3)]"
    />
  )
