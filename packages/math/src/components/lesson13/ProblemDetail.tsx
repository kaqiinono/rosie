...
import { TAG_STYLE } from '@rosie/math/utils/lesson13-data'
...
export default function ProblemDetail({ problem, mode = 'full', defaultSolutionOpen = false }: ProblemDetailProps) {
  ...
  const { solveCount, handleSolve, addWrong } = useLesson13()
  ...
  const solution = (
    <div className="mb-3.5 rounded-lg border border-green-200 bg-gradient-to-br from-green-50 to-[#dcfce7] p-3.5">
      <div className="mb-1.5 flex items-center gap-1 text-xs font-bold text-green-700">
        🔍 题型分析
      </div>
      <ul className="flex flex-col gap-1.5">
        {problem.analysis.map((a, i) => (
          <li key={i} className="flex items-start gap-1.5 text-xs leading-relaxed text-green-900">
    ...
    <>
      ...
      <div className="mb-3 rounded-lg border border-dashed border-border-light bg-[#f9fafb] p-3.5">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
          ...
          <button
            onClick={checkAnswer}
            className="cursor-pointer rounded-full bg-green-600 px-4 py-2 text-[13px] font-semibold text-white shadow-[0_3px_10px_rgba(22,163,74,0.3)] transition-all active:translate-y-px"
          >
            检查答案
          </button>
        </div>
