...
export default function FilterPanel({ problems, solveCount, filters, onToggleFilter, onSetMastery }: FilterPanelProps) {
  ...
  return (
    <div>
      <div className="mb-3 rounded-[14px] border border-[#fcd34d] bg-gradient-to-br from-[#fffbeb] to-amber-50 p-4">
        <div className="mb-1.5 text-[15px] font-extrabold text-amber-800">🎯 综合测试题库</div>
        <div className="mb-2.5 text-xs text-amber-700">全部{total}道题 · 多选筛选 · 按题型/来源练习</div>

        <div className="mb-2">
          <div className="mb-1.5 flex items-center justify-between">
            <span className="text-[11px] font-bold text-amber-700">📂 来源筛选</span>
            <button onClick={() => SOURCE_BTNS.forEach(b => { if (allSourceSelected || !filters.source.has(b.key)) onToggleFilter('source', b.key) })} className="cursor-pointer text-[10px] text-amber-500 hover:text-amber-700 transition-colors">{allSourceSelected ? '全不选' : '全选'}</button>
          </div>
        ...
        <div className="mb-2">
          <div className="mb-1.5 flex items-center justify-between">
            <span className="text-[11px] font-bold text-amber-700">🏷️ 题型筛选</span>
            <button onClick={() => TYPE_BTNS.forEach(b => { if (allTypeSelected || !filters.type.has(b.key)) onToggleFilter('type', b.key) })} className="cursor-pointer text-[10px] text-amber-500 hover:text-amber-700 transition-colors">{allTypeSelected ? '全不选' : '全选'}</button>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {TYPE_BTNS.map(b => (
              <button
...
<div className="mb-2">
          <div className="mb-1.5 text-[11px] font-bold text-amber-700">🎯 掌握度筛选</div>
          <div className="flex flex-wrap gap-1.5">
            {MASTERY_BTNS.map(b => (
              <button
                key={b.key}
        ...
        <div className="mt-2 space-y-1.5">
          <div className="flex items-center gap-1.5 text-[11px] text-amber-700">
            <span>练过 <strong className="text-amber-800">{attempted}</strong> 道</span>
            <span className="text-amber-300">·</span>
            <span>🦋 掌握 <strong className="text-amber-800">{mastered}</strong> 道</span>
            <span className="text-amber-300">·</span>
            <span>共 {total} 题</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="relative h-[6px] flex-1 overflow-hidden rounded-full bg-amber-100">
              <div
