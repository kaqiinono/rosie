...
import { TAG_STYLE } from '@/utils/lesson45-data'
...
export default function ProblemDetail({ problem, mode = 'full', defaultSolutionOpen = false }: ProblemDetailProps) {
  ...
  const { solveCount, handleSolve, addWrong } = useLesson45()
  ...
  const solution = (
    <div className="mb-3.5 rounded-lg border border-orange-200 bg-gradient-to-br from-orange-50 to-[#ffedd5] p-3.5">
      <div className="mb-1.5 flex items-center gap-1 text-xs font-bold text-orange-700">
        🔍 题型分析
      </div>
      <ul className="flex flex-col gap-1.5">
        {problem.analysis.map((a, i) => (
          <li key={i} className="flex items-start gap-1.5 text-xs leading-relaxed text-orange-900">
  ...
  const question = (
    <div className="flex flex-col gap-1.5">
      <div className="min-w-0 flex-1">
        ...
        <div
          className="mb-3.5 rounded-lg border-l-3 border-orange-300 bg-orange-50 px-3.5 py-3 text-sm leading-relaxed text-text-secondary [&>strong]:font-bold [&>strong]:text-text-primary"
    ...
    <>
      ...
      <div className="mb-3 rounded-lg border border-dashed border-border-light bg-[#f9fafb] p-3.5">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
          ...
          <button
            onClick={checkAnswer}
            className="cursor-pointer rounded-full bg-orange-600 px-4 py-2 text-[13px] font-semibold text-white shadow-[0_3px_10px_rgba(234,88,12,0.3)] transition-all active:translate-y-px"
          >
            检查答案
          </button>
        </div>
