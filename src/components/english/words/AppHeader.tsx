...
export default function AppHeader({onImport, onExport, onImmersive}: AppHeaderProps) {
  ...
  return (
    <header
      className="sticky top-0 z-50 bg-[var(--wm-bg)]/95 backdrop-blur-xl border-b border-[var(--wm-border)] px-4 py-2.5">
      <div className="max-w-[1280px] mx-auto flex items-center justify-between gap-2.5 flex-wrap">
        <Link href={'/'}>
          <div
            className="font-bold font-fredoka text-2xl bg-gradient-to-br from-[var(--wm-accent)] to-[var(--wm-accent2)] bg-clip-text text-transparent whitespace-nowrap">
            📚 Rosie Fun
          </div>
        </Link>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <div ref={ddRef} className="relative">
            ...
            {menuOpen && (
              <div
                className="absolute top-[calc(100%+8px)] right-0 bg-[#1e1e35] border border-white/10 rounded-xl overflow-hidden min-w-[160px] shadow-[0_8px_32px_rgba(0,0,0,.5)] z-[200] animate-[drop-in_.18s_cubic-bezier(.4,0,.2,1)]">
                <button
                  onClick={() => {
                    onImport();
                    setMenuOpen(false)
