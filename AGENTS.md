...
`/setting/calc` manages named口算策略. A user may create multiple strategies, but at most one is
enabled; that enabled row is the default used by normal口算 and direct entry from 今日计划.
