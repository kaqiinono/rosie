...
import { TAG_STYLE, TYPE_TIP } from '@rosie/math/utils/lesson55-data'
...
export default function ProblemDetail({
  ...
  const { solveCount, handleSolve, addWrong } = useLesson55()
  ...
  function checkAnswer() {
    ...
    if (v === problem.finalAns) {
      ...
      setFeedback({ text: '❌ 不对哦，再想想？提示：分类列举，注意有序还是无序、是否相同。', ok: false })
      addWrong(problem.id)
    }
  }
      ...
      <div className="mb-3 rounded-lg border border-dashed border-border-light bg-[#f9fafb] p-3.5">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
          ...
          <button onClick={checkAnswer}
            className="cursor-pointer rounded-full bg-teal-600 px-4 py-2 text-[13px] font-semibold text-white shadow-[0_3px_10px_rgba(14,165,233,0.3)] transition-all active:translate-y-px">
            检查答案
          </button>
        </div>
        {feedback && (
