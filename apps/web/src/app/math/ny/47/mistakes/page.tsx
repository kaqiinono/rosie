...
export default function MistakesPage() {
  ...
  return (
    <div>
      <div className="mb-3.5 rounded-[14px] border border-[#fecaca] bg-gradient-to-br from-[#fff5f5] to-[#fae8ff] p-4">
        ...
        <div className="mb-2 text-xs text-[#b91c1c]">没做出来的方格谜题会收录在这里 · 再做出来后移除</div>
        {wrongList.length > 0 && (
          <div className="flex items-center gap-2">
            <div className="relative h-[5px] flex-1 overflow-hidden rounded-full bg-[#fecaca]">
              <div className="absolute inset-y-0 left-0 rounded-full bg-[#f87171] transition-[width] duration-500"
      ...
      {wrongList.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-3 py-16 text-center">
          ...
          <div className="text-[13px] text-text-muted">放心大胆地去拼方格谜题吧</div>
          <Link
            href="/math/ny/47"
            className="mt-2 rounded-full bg-fuchsia-500 px-5 py-2 text-[13px] font-semibold text-white no-underline shadow-[0_3px_10px_rgba(192,38,211,0.3)]"
          >
            去做题 →
          </Link>
        </div>
        ...
        <div className="flex flex-col gap-2">
          {wrongList.map(({ p, setName, idx }) => {
            ...
            const href = `/math/ny/47/${setName}/${idx + 1}`
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 flex flex-wrap gap-1">
                    ...
                    <span className="rounded-full bg-fuchsia-100 px-2 py-px text-[10px] font-semibold text-fuchsia-700">
                      {srcLabel}
                    </span>
                    {count > 0 && (
                ...
                <div className="flex shrink-0 items-center gap-1.5">
                  <Link href={href}
                    className="rounded-full bg-fuchsia-500 px-3 py-1.5 text-[11px] font-semibold text-white no-underline">
                    {isMastered ? '再练' : '去练'}
                  </Link>
                  <button onClick={() => removeWrong(p.id)}
                    className="rounded-full border border-[#fca5a5] px-2 py-1.5 text-[11px] text-[#dc2626] transition-colors hover:bg-[#fae8ff]"
                    title="从错题本移除">
                    ✕
                  </button>
                </div>
