...
import Lesson30Provider, { useLesson30 } from '@/components/math/lesson30/Lesson30Provider'
import AppHeader from '@/components/math/lesson30/AppHeader'
import Sidebar from '@/components/math/lesson30/Sidebar'
import BottomNav from '@/components/math/lesson30/BottomNav'
...
function getNextHref(pathname: string): string | undefined {
  ...
  return `/math/ny/30/${section}/${index + 1}`
}

function InnerLayout({ children }: { children: React.ReactNode }) {
  const { toast, setToast, showCongrats, setShowCongrats } = useLesson30()
  const pathname = usePathname()
  const nextHref = getNextHref(pathname)

  return (
    <div
      className="flex min-h-screen flex-col bg-amber-50 text-[15px] text-text-primary"
...
export default function Lesson30Layout({ children }: { children: React.ReactNode }) {
  return (
    <Lesson30Provider>
