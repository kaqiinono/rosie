...
import { TAG_STYLE } from '@rosie/math/utils/lesson30-data'
...
export default function ProblemDetail({ problem, mode = 'full', defaultSolutionOpen = false }: ProblemDetailProps) {
  ...
  const { solveCount, handleSolve, addWrong } = useLesson30()
    ...
    <>
      ...
      <div className="mb-3 rounded-lg border border-dashed border-border-light bg-[#f9fafb] p-3.5">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
          ...
          <button
            onClick={checkAnswer}
            className="cursor-pointer rounded-full bg-amber-600 px-4 py-2 text-[13px] font-semibold text-white shadow-[0_3px_10px_rgba(217,119,6,0.3)] transition-all active:translate-y-px"
          >
            检查答案
          </button>
        </div>
