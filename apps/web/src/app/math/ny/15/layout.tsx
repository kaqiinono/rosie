...
import Lesson15Provider, { useLesson15 } from '@rosie/math/components/lesson15/Lesson15Provider'
import AppHeader from '@rosie/math/components/lesson15/AppHeader'
import Sidebar from '@rosie/math/components/lesson15/Sidebar'
import LessonInnerLayout from '@rosie/math/components/shared/LessonInnerLayout'
import BottomNav from '@rosie/math/components/lesson15/BottomNav'
...
function getNextHref(pathname: string): string | undefined {
  ...
  return `/math/ny/15/${section}/${index + 1}`
}

function InnerLayout({ children }: { children: React.ReactNode }) {
  const { toast, setToast, showCongrats, setShowCongrats } = useLesson15()
  const pathname = usePathname()
  const nextHref = getNextHref(pathname)

  return (
    <div
      className="flex min-h-screen flex-col bg-sky-50 text-[15px] text-text-primary"
...
export default function Lesson15Layout({ children }: { children: React.ReactNode }) {
  return (
    <Lesson15Provider>
