...
import { PROBLEMS, TAG_STYLE } from '@/utils/lesson29-data'
...
export default function MistakesPage() {
  ...
  return (
    <div>
      <div className="mb-3.5 rounded-[14px] border border-[#fecaca] bg-gradient-to-br from-[#fff5f5] to-[#ffe4e6] p-4">
      ...
      {wrongList.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {wrongList.map(({ p, setName, idx }) => {
            ...
            const href = `/math/ny/29/${setName}/${idx + 1}`
            ...
            return (
              ...
              >
                ...
                <div className="flex shrink-0 items-center gap-1.5">
                  <Link href={href}
                    className="rounded-full bg-rose-500 px-3 py-1.5 text-[11px] font-semibold text-white no-underline">
                    {isMastered ? '再练' : '去练'}
                  </Link>
                  <button onClick={() => removeWrong(p.id)}
                    className="rounded-full border border-[#fca5a5] px-2 py-1.5 text-[11px] text-[#dc2626] transition-colors hover:bg-[#ffe4e6]"
                    title="从错题本移除">
                    ✕
                  </button>
                </div>
