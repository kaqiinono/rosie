...
import ProblemList from '@rosie/math/components/lesson55/ProblemList'
  ...
  return (
    <div>
      <div className="mb-3.5 rounded-[14px] border border-yellow-200 bg-gradient-to-br from-yellow-50 to-[#fef9c3] p-4">
        <div className="mb-1 text-sm font-extrabold text-yellow-900">📝 课前测 · 第55讲</div>
        <div className="mb-2 text-xs text-yellow-700">{total > 0 ? `${total}道摸底题 · 检验起始水平` : '本讲暂无课前测'}</div>
      ...
      <ProblemList problems={list} solveCount={solveCount} basePath="/math/ny/55/pretest" />
    </div>
  )
}
