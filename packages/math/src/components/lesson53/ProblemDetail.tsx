...
import { TAG_STYLE, TYPE_TIP } from '@rosie/math/utils/lesson53-data'
...
export default function ProblemDetail({
  ...
  const { solveCount, handleSolve, addWrong } = useLesson53()
  ...
  function checkAnswer() {
    ...
    if (v === problem.finalAns) {
      ...
      setFeedback({ text: '❌ 不对哦，再想想？提示：先看相邻项的差、比或和。', ok: false })
      addWrong(problem.id)
    }
  }
  ...
  const question = (
    <div className="flex flex-col gap-1.5">
      <div className="min-w-0 flex-1">
        ...
        <div
          className="mb-3.5 rounded-lg border-l-3 border-amber-300 bg-amber-50 px-3.5 py-3 text-sm leading-relaxed text-text-secondary [&>strong]:font-bold [&>strong]:text-text-primary"
      ...
      <div className="mb-3 rounded-lg border border-dashed border-border-light bg-[#f9fafb] p-3.5">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
          ...
          <button onClick={checkAnswer}
            className="cursor-pointer rounded-full bg-amber-600 px-4 py-2 text-[13px] font-semibold text-white shadow-[0_3px_10px_rgba(245,158,11,0.3)] transition-all active:translate-y-px">
            检查答案
          </button>
        </div>
        {feedback && (
