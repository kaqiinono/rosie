      ...
      <div className="mb-3 rounded-[14px] border border-rose-200 bg-gradient-to-br from-rose-50 to-[#fee2e2] p-4">
        ...
        <div className="mb-2">
          <div className="mb-1.5 flex items-center justify-between">
            <span className="text-[11px] font-bold text-rose-700">📂 来源筛选</span>
            <button onClick={() => SOURCE_BTNS.forEach(b => { if (allSourceSelected || !filters.source.has(b.key)) onToggleFilter('source', b.key) })} className="cursor-pointer text-[10px] text-rose-500 hover:text-rose-700 transition-colors">{allSourceSelected ? '全不选' : '全选'}</button>
          </div>
        ...
        <div className="mb-2">
          <div className="mb-1.5 flex items-center justify-between">
            <span className="text-[11px] font-bold text-rose-700">🏷️ 题型筛选</span>
            <button onClick={() => TYPE_BTNS.forEach(b => { if (allTypeSelected || !filters.type.has(b.key)) onToggleFilter('type', b.key) })} className="cursor-pointer text-[10px] text-rose-500 hover:text-rose-700 transition-colors">{allTypeSelected ? '全不选' : '全选'}</button>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {TYPE_BTNS.map(b => (
