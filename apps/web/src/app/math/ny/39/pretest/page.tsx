...
import ProblemList from '@rosie/math/components/lesson39/ProblemList'
  ...
  return (
    <div>
      <div className="mb-3.5 rounded-[14px] border border-green-200 bg-gradient-to-br from-green-50 to-[#dcfce7] p-4">
        <div className="mb-1 text-sm font-extrabold text-green-900">📝 课前测 · 第39讲</div>
        <div className="mb-2 text-xs text-green-700">{total > 0 ? `${total}道测试题 · 基础检测` : '题目即将上线，敬请期待'}</div>
        {total > 0 && (
      ...
      <ProblemList problems={list} solveCount={solveCount} basePath="/math/ny/39/pretest" />
    </div>
  )
}
