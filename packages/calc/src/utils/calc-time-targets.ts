...
export const TIME_TARGETS: Record<string, TimeTarget> = {
  ...
  'mul:2d': { entry: [20, 25], stable: [18, 22], fluent: [15, 18], auto: [10, 12] },
  // 除法
  ...
  'div:rem': { entry: [12, 15], stable: [9, 11], fluent: [6, 8], auto: [4, 5] },
  // 小数
  ...
  // 分数
  'frac:add-same': { entry: [8, 10], stable: [5, 6], fluent: [4, 5], auto: [3, 3] },
  // 异分母加减：入门/进阶/高级放宽；分数除分数同为高年级内容，auto 档放宽为冲刺目标
  'frac:add-diff': { entry: [35, 40], stable: [25, 30], fluent: [20, 25], auto: [15, 18] },
  ...
  'frac:div-frac': { entry: [25, 30], stable: [20, 25], fluent: [15, 20], auto: [15, 18] },
  // 混合骨架
