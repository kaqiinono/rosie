...
import { TAG_STYLE } from '@rosie/math/utils/lesson46-data'
...
export default function ProblemDetail({ problem, mode = 'full', defaultSolutionOpen = false, prevHref = null, nextHref = null, positionLabel }: ProblemDetailProps) {
  const { solveCount, handleSolve, addWrong } = useLesson46()
  ...
  const { answer, setAnswer, feedback, check } = useProblemAnswer(problem, {
    ...
    wrongHint: '🤔 再想想？先把"最倒霉"的情况全拿完，再加 1 试试。',
  })

  const solution = (
    <div className="mb-3.5 rounded-lg border border-teal-200 bg-gradient-to-br from-teal-50 to-[#ccfbf1] p-3.5">
      <div className="mb-1.5 flex items-center gap-1 text-xs font-bold text-teal-700">
        🔍 题型分析
      </div>
      <ul className="flex flex-col gap-1.5">
        {problem.analysis.map((a, i) => (
          <li
            key={i}
            className="flex items-start gap-1.5 text-xs leading-relaxed text-teal-900 [&>strong]:font-bold"
          >
  ...
  const question = (
    <div className="flex flex-col gap-1.5">
      <div className="min-w-0 flex-1">
        ...
        <div
          className="mb-3.5 rounded-lg border-l-3 border-teal-300 bg-teal-50 px-3.5 py-3 text-sm leading-relaxed text-text-secondary [&>strong]:font-bold [&>strong]:text-text-primary"
  ...
  const answerDom = (
    <NumericAnswerPanel
      ...
      onCheck={check}
      feedback={feedback}
      buttonClassName="bg-teal-600 shadow-[0_3px_10px_rgba(13,148,136,0.3)]"
    />
  )
