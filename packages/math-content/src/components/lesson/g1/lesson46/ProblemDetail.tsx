...
import { TAG_STYLE } from '@rosie/math-content/utils/g1/lesson46-data'
...
export default function ProblemDetail({
  ...
  const { practiceCount, correctCount, handleSolve, addWrong } = useG1Lesson46()
  const count = practiceCount[problem.id] ?? 0
  const level = getMasteryLevel(correctCount[problem.id] ?? 0)

  const { answer, setAnswer, feedback, check, hasAttempted } = useProblemAnswer(
    problem,
    {
      handleSolve,
      addWrong,
  ...
  const solution = <ProblemSolutionPanel problem={problem} variant="teal" />
