...
- 古诗 / 日积月累不单独作为本计划必选题型（沿用现有 practice session 能力即可）。
...
- 当前批次内，计划勾选的题型均完成 → 对应关写入 `completed_lesson_keys`，`current_lesson_key` 移到下一未完成关。
...
| 换册 | **禁止**；另建计划 |
| 改题型导致「未完成」定义变化 | 推进只看当前 `quiz_types`；历史 run 保留当时快照 |
