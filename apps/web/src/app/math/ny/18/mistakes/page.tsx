...
import { PROBLEMS, TAG_STYLE } from '@/utils/lesson18-data'
...
export default function MistakesPage() {
  ...
  return (
    <div>
      <div className="mb-3.5 rounded-[14px] border border-[#fecaca] bg-gradient-to-br from-[#fff5f5] to-[#ede9fe] p-4">
      ...
      {wrongList.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-3 py-16 text-center">
          ...
          <Link
            href="/math/ny/18"
            className="mt-2 rounded-full bg-purple-500 px-5 py-2 text-[13px] font-semibold text-white no-underline shadow-[0_3px_10px_rgba(147,51,234,0.3)]"
          >
            去做题 →
          </Link>
        </div>
        ...
        <div className="flex flex-col gap-2">
          {wrongList.map(({ p, setName, idx }) => {
            ...
            const href = `/math/ny/18/${setName}/${idx + 1}`
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 flex flex-wrap gap-1">
                    ...
                    <span className="rounded-full bg-purple-100 px-2 py-px text-[10px] font-semibold text-purple-700">
                      {srcLabel}
                    </span>
                    {count > 0 && (
                ...
                <div className="flex shrink-0 items-center gap-1.5">
                  <Link href={href}
                    className="rounded-full bg-purple-500 px-3 py-1.5 text-[11px] font-semibold text-white no-underline">
                    {isMastered ? '再练' : '去练'}
                  </Link>
                  <button onClick={() => removeWrong(p.id)}
                    className="rounded-full border border-[#fca5a5] px-2 py-1.5 text-[11px] text-[#dc2626] transition-colors hover:bg-[#ede9fe]"
                    title="从错题本移除">
                    ✕
                  </button>
                </div>
