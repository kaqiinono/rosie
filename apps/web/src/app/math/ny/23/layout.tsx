...
import Lesson23Provider, { useLesson23 } from '@rosie/math/components/lesson23/Lesson23Provider'
import AppHeader from '@rosie/math/components/lesson23/AppHeader'
import Sidebar from '@rosie/math/components/lesson23/Sidebar'
import BottomNav from '@rosie/math/components/lesson23/BottomNav'
...
function getNextHref(pathname: string): string | undefined {
  ...
  return `/math/ny/23/${section}/${index + 1}`
}

function InnerLayout({ children }: { children: React.ReactNode }) {
  const { toast, setToast, showCongrats, setShowCongrats } = useLesson23()
  const pathname = usePathname()
  const nextHref = getNextHref(pathname)

  return (
    <div
      className="flex min-h-screen flex-col bg-violet-50 text-[15px] text-text-primary"
...
export default function Lesson23Layout({ children }: { children: React.ReactNode }) {
  return (
    <Lesson23Provider>
