...
import { TAG_STYLE } from '@rosie/math/utils/g1/lesson42-data'
...
export default function ProblemDetail({
  ...
  const { solveCount, handleSolve, addWrong } = useG1Lesson42()
  ...
  const solution = <ProblemSolutionPanel problem={problem} variant="rose" />
  ...
  const question = (
    <div className="flex flex-col gap-1.5">
      <div className="min-w-0 flex-1">
        ...
        <div
          className="text-text-secondary [&>strong]:text-text-primary mb-3.5 rounded-lg border-l-3 border-rose-300 bg-rose-50 px-3.5 py-3 text-sm leading-relaxed [&>strong]:font-bold"
  ...
  const answerDom = (
    <NumericAnswerPanel
      ...
      onCheck={check}
      feedback={feedback}
      buttonClassName="bg-rose-600 shadow-[0_3px_10px_rgba(190,18,60,0.3)]"
    />
  )
