#!/usr/bin/env python3
"""Generate docs/sql/chinese-g1b/*.sql from g1b TS backup + hanzi metadata.
...
REPO = PKG.parents[1]
OUT_DIR = REPO / "docs/sql/chinese-g1b"
LEGACY_SQL = REPO / "docs/sql/chinese-g1b-upsert.sql"
...
def load_radical_names() -> dict[str, str]:
    radicals = load_json_export(PKG / "src/utils/g1b/radicals.ts", "RADICALS")
...
def main() -> int:
    ...
    readme = [
        "# 一年级下册字库灌库（分片 SQL）",
        "",
        "单文件 `chinese-g1b-upsert.sql` 因笔顺 JSON 过大，Supabase SQL Editor 无法执行。",
    ...
    readme += [
        ...
        "有 psql 时可一键导入：",
        "```bash",
        "for f in docs/sql/chinese-g1b/*.sql; do psql \"$DATABASE_URL\" -f \"$f\"; done",
        "```",
    ]
    write_sql(OUT_DIR / "README.md", readme)

    legacy_note = [
        "-- 此文件已拆分为 docs/sql/chinese-g1b/ 目录下的多个小文件。",
        "-- Supabase SQL Editor 无法运行 ~1.3MB 的单条灌库脚本。",
        "-- 请阅读 docs/sql/chinese-g1b/README.md 并按顺序执行各分片。",
        "",
    ]
