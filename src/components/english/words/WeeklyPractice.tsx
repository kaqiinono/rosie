...
export default function WeeklyPractice({ vocab }: WeeklyPracticeProps) {
  ...
  const effectivePhase = useMemo(() => {
    if (!showSetup && phase === 'setup' && weeklyPlan && !isLoading) return 'week-view'
    return phase
  }, [phase, weeklyPlan, isLoading, showSetup])
    ...
    return (
      <div className="max-w-[560px] mx-auto px-4 py-10">
        <div className="bg-[var(--wm-surface)] border border-[var(--wm-border)] rounded-[20px] p-7">
          <div className="font-fredoka text-2xl bg-gradient-to-br from-[#f59e0b] to-[#f97316] bg-clip-text text-transparent mb-1">
            本周计划
          </div>
          ...
          {activeLesson ? (
            <>
              ...
              <div className="mt-5 pt-4 border-t border-[var(--wm-border)]">
                <div className="text-[.68rem] font-extrabold text-[var(--wm-text-dim)] uppercase tracking-widest mb-2.5">每天新词数量</div>
                ...
                <div className="text-[.68rem] font-extrabold text-[var(--wm-text-dim)] uppercase tracking-widest mb-2.5">每周开始于</div>
      ...
      <div className="max-w-[1280px] mx-auto px-4 py-5">
        <div className="bg-[var(--wm-surface)] border border-[var(--wm-border)] rounded-[20px] p-7 mb-5">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap mb-5">
            <div>
              <div className="font-fredoka text-2xl bg-gradient-to-br from-[#f59e0b] to-[#f97316] bg-clip-text text-transparent">
          ...
          <div className="flex gap-2 mb-5 overflow-x-auto pb-1 sm:grid sm:grid-cols-7 sm:overflow-visible sm:pb-0 scrollbar-none">
            {weeklyPlan.days.map((day, i) => {
              ...
              if (isDone) { borderColor = '#4ade80'; textColor = '#4ade80'; bgColor = 'rgba(74,222,128,.08)' }
              else if (isToday) { borderColor = '#f59e0b'; textColor = '#fbbf24'; bgColor = 'rgba(245,158,11,.1)' }
              else if (isPast) { borderColor = 'rgba(248,113,113,.3)'; textColor = 'rgba(248,113,113,.6)' }
              if (isSelected) { bgColor = isDone ? 'rgba(74,222,128,.2)' : isToday ? 'rgba(245,158,11,.2)' : 'rgba(255,255,255,.08)' }
            ...
            return (
              <div className="border-t border-[var(--wm-border)] pt-4">
                ...
                <div className="flex items-center gap-2 flex-wrap mb-3">
                  ...
                  <span className="bg-[rgba(249,115,22,.15)] text-[#fb923c] border border-[rgba(249,115,22,.3)] px-2 py-0.5 rounded-full text-[.65rem] font-bold">新词 {newWords.length}</span>
                  {weekReview.length > 0 && <span className="bg-[rgba(96,165,250,.15)] text-[#93c5fd] border border-[rgba(96,165,250,.3)] px-2 py-0.5 rounded-full text-[.65rem] font-bold">本周复习 {weekReview.length}</span>}
                  {oldReview.length > 0 && <span className="bg-[rgba(167,139,250,.15)] text-[#c4b5fd] border border-[rgba(167,139,250,.3)] px-2 py-0.5 rounded-full text-[.65rem] font-bold">旧词 {oldReview.length}</span>}
                  <span className="text-[.68rem] text-[var(--wm-text-dim)]">共 {total} 词</span>
                  {isDone && weeklyPlan.progress[selectedDate]?.lastScore !== undefined && (
                    <span className="ml-auto text-[.72rem] font-bold text-[#4ade80]">上次 {weeklyPlan.progress[selectedDate].lastScore}%</span>
                  )}
                </div>
                ...
                {newWords.length > 0 && (
                  <div className="mb-3">
                    <div className="text-[.6rem] font-extrabold text-[#fb923c] uppercase tracking-widest mb-1.5">今日新词</div>
                    <div className="flex flex-wrap gap-1.5">
                      {newWords.map(w => {
                        const level = getMasteryLevel(masteryMap[wordKey(w)]?.correct ?? 0)
                        return (
                          <span key={wordKey(w)} className="px-2.5 py-1 rounded-full border-[1.5px] border-[rgba(249,115,22,.4)] bg-[rgba(249,115,22,.08)] text-[#fb923c] text-[.78rem] font-bold">
                ...
                {weekReview.length > 0 && (
                  <div className="mb-3">
                    <div className="text-[.6rem] font-extrabold text-[#93c5fd] uppercase tracking-widest mb-1.5">本周复习</div>
                    <div className="flex flex-wrap gap-1.5">
                      {weekReview.map(w => {
                        const level = getMasteryLevel(masteryMap[wordKey(w)]?.correct ?? 0)
                        return (
                          <span key={wordKey(w)} className="px-2.5 py-1 rounded-full border-[1.5px] border-[rgba(96,165,250,.35)] bg-[rgba(96,165,250,.08)] text-[#93c5fd] text-[.78rem] font-bold">
                            {level > 0 && <span className="mr-1 text-[.65rem]">{MASTERY_ICON[level]}</span>}{w.word}
                          </span>
                        )
                      })}
                ...
                {oldReview.length > 0 && (
                  <div className="mb-4">
                    <div className="text-[.6rem] font-extrabold text-[#c4b5fd] uppercase tracking-widest mb-1.5">旧词复习</div>
                    <div className="flex flex-wrap gap-1.5">
                      {oldReview.map(w => {
                        const level = getMasteryLevel(masteryMap[wordKey(w)]?.correct ?? 0)
                        return (
                          <span key={wordKey(w)} className="px-2.5 py-1 rounded-full border-[1.5px] border-[rgba(167,139,250,.35)] bg-[rgba(167,139,250,.08)] text-[#c4b5fd] text-[.78rem] font-bold">
                            {level > 0 && <span className="mr-1 text-[.65rem]">{MASTERY_ICON[level]}</span>}{w.word}
                          </span>
                        )
                      })}
                ...
                <div className="text-[.68rem] font-extrabold text-[var(--wm-text-dim)] uppercase tracking-widest mb-2.5">题型选择</div>
  ...
  if (effectivePhase === 'quiz' && quizQs[curQ]) {
    ...
    const options = [...pool, q.word].sort(() => Math.random() - .5)

    return (
      <div className="max-w-[1280px] mx-auto px-4 py-5">
        <div className="flex items-center gap-3 py-3 mb-2">
          <button
            onClick={() => { setStudyIdx(0); setStudyWordVisible(false); setPhase('study') }}
    ...
    return (
      ...
      <div className="max-w-[500px] mx-auto py-10 px-5 text-center">
        ...
        <div className="flex gap-2.5 justify-center flex-wrap">
          ...
          <button onClick={() => setPhase('week-view')} className="px-5 py-2.5 bg-transparent border-[1.5px] border-[var(--wm-border)] rounded-[10px] text-[var(--wm-text-dim)] font-nunito font-bold text-[.82rem] cursor-pointer">
            返回本周
          </button>
        </div>
      </div>
