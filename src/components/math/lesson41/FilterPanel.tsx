      ...
      <button onClick={() => onToggle(cardId)} className="flex w-full cursor-pointer items-center gap-2.5 rounded-[12px] p-3 text-left">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-0.5 flex flex-wrap gap-1">
            ...
            <span className="rounded-full bg-[#e0f2fe] px-2 py-px text-[10px] font-semibold text-[#0369a1]">{srcLabel}</span>
          </div>
        </div>
...
export default function FilterPanel({ problems, solveCount, filters, onToggleFilter, onSetMastery }: FilterPanelProps) {
  ...
  const btnOn  = 'border-sky-600 bg-sky-600 text-white'
  const btnOff = 'border-sky-300 bg-[#f0f9ff] text-sky-700'

  return (
    <div>
      <div className="mb-3 rounded-[14px] border border-sky-200 bg-gradient-to-br from-[#f0f9ff] to-[#e0f2fe] p-4">
        <div className="mb-1.5 text-[15px] font-extrabold text-sky-800">🎯 综合题库 · 第41讲</div>
        <div className="mb-2.5 text-xs text-sky-700">全部{total}道题 · 多选筛选 · 按题型/来源练习</div>

        <div className="mb-2">
          <div className="mb-1.5 flex items-center justify-between">
            <span className="text-[11px] font-bold text-sky-700">📂 来源筛选</span>
            <button onClick={() => SOURCE_BTNS.forEach(b => { if (allSourceSelected || !filters.source.has(b.key)) onToggleFilter('source', b.key) })} className="cursor-pointer text-[10px] text-sky-500 hover:text-sky-700 transition-colors">{allSourceSelected ? '全不选' : '全选'}</button>
          </div>
        ...
        <div className="mb-2">
          <div className="mb-1.5 flex items-center justify-between">
            <span className="text-[11px] font-bold text-sky-700">🏷️ 题型筛选</span>
            <button onClick={() => TYPE_BTNS.forEach(b => { if (allTypeSelected || !filters.type.has(b.key)) onToggleFilter('type', b.key) })} className="cursor-pointer text-[10px] text-sky-500 hover:text-sky-700 transition-colors">{allTypeSelected ? '全不选' : '全选'}</button>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {TYPE_BTNS.map(b => (
...
<div className="mb-2">
          <div className="mb-1.5 text-[11px] font-bold text-sky-700">🎯 掌握度</div>
          <div className="flex flex-wrap gap-1.5">
            {MASTERY_BTNS.map(b => (
        ...
        <div className="mt-2 space-y-1.5">
          <div className="flex items-center gap-1.5 text-[11px] text-sky-700">
            <span>练过 <strong className="text-sky-800">{attempted}</strong> 道</span>
            <span className="text-sky-300">·</span>
            <span>🦋 掌握 <strong className="text-sky-800">{mastered}</strong> 道</span>
            <span className="text-sky-300">·</span>
            <span>共 {total} 题</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="relative h-[6px] flex-1 overflow-hidden rounded-full bg-sky-100">
            ...
            return (
              <Link key={p.id} href={getProblemHref(setName, idx)}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 flex flex-wrap gap-1">
                    <span className={`rounded-full px-2 py-px text-[10px] font-semibold ${TAG_COLORS[p.tag] || 'bg-gray-100 text-gray-600'}`}>{p.tagLabel}</span>
                    <span className="rounded-full bg-[#e0f2fe] px-2 py-px text-[10px] font-semibold text-[#0369a1]">{SOURCE_LABELS[setName] || setName}</span>
                  </div>
                </div>
