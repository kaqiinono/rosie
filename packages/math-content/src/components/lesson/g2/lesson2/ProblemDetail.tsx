...
import { TAG_STYLE, TYPE_TIP } from '@rosie/math-content/utils/g2/lesson2-data'
...
export default function ProblemDetail({
  ...
  const { practiceCount, correctCount, handleSolve, addWrong } = useG2Lesson2()
  ...
  const tipBlock = tipText ? (
    <div className="mt-3 rounded-lg bg-teal-50 px-3 py-2.5 text-xs leading-relaxed text-teal-800">
      💡 <strong>巧算口诀：</strong>
      {tipText}
    </div>
  ) : null
  ...
  const answerDom = (
    <ProblemAnswerSection
      ...
      buttonClassName="bg-teal-600 shadow-[0_3px_10px_rgba(15,118,110,0.3)]"
      tip={tipBlock}
    />
  )
