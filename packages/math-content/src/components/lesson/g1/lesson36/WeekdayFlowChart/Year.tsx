...
import Month from '@rosie/math-content/components/lesson/g1/lesson36/WeekdayFlowChart/Month'
...
const NumInput: React.FC<{
  ...
  <input
    ...
    className={`rounded-md border-none text-center font-bold outline-none ${className}`}
...
const DateBox = memo(({ label, isFixed }: { label: string; isFixed: boolean }) => (
  <div
    className={`flex items-center justify-center rounded-2xl text-sm font-semibold select-none ${isFixed ? 'bg-blue-100 text-blue-700' : 'bg-blue-50 text-blue-500'}`}
...
const WeekdayPill = memo(
  ({
    ...
    const wdName =
      value && +value >= 1 && +value <= 7 ? WD[+value as 1 | 2 | 3 | 4 | 5 | 6 | 7] : null
    const ring = correct ? 'ring-2 ring-emerald-400' : wrong ? 'ring-2 ring-red-400' : ''

    return (
      <div
        className={`flex items-center justify-center rounded-full bg-rose-100 text-sm font-bold text-rose-500 ${ring}`}
        ...
        {fixedLabel ? (
          ...
          <>
            ...
            {onChange && (
              <input
                value={value}
                onChange={(e) => onChange(e.target.value)}
                className="absolute inset-0 z-10 h-full w-full cursor-text rounded-full opacity-0"
          ...
          <div className="flex items-center gap-0.5">
            ...
            <input
              ...
              onChange={(e) => onChange?.(e.target.value)}
              placeholder="?"
              className="w-6 border-none bg-transparent text-center text-xs font-bold text-rose-500 outline-none placeholder:text-rose-200"
    ...
    return (
      <div className="flex" style={{ height: ARROW_H }}>
        ...
        >
          ...
          <div
            className={`flex items-center gap-0.5 rounded-lg px-2.5 text-xs font-bold ${
              daysOk
        ...
        >
          {direction === 'backward' && arrowSvg}
          <div
            className={`flex items-center gap-0.5 rounded-lg px-2 text-xs font-bold ${
              plusOk
                ? 'bg-emerald-50 text-emerald-600 ring-1 ring-emerald-300'
                : plusErr
                  ? 'bg-red-50 text-red-500 ring-1 ring-red-300'
...
const MNum: React.FC<{
  ...
  <input
    ...
    onChange={(e) => onChange(e.target.value)}
    placeholder="?"
    className={`rounded-md border-none text-center text-[12px] font-bold outline-none ${cls}`}
    style={{ width, height: 22, fontFamily: FONT }}
  />
)

const MDaysPill = memo(
  ({
    value,
    onChange,
    isEndpoint,
    ...
    return (
      <div
        className={`flex items-center gap-1 rounded-full px-2.5 shadow-sm ${bg} ${ring}`}
...
const MWdPill = memo(
  ({
    ...
    const wdName =
      value && +value >= 1 && +value <= 7 ? MONTH_WD[+value as 1 | 2 | 3 | 4 | 5 | 6 | 7] : null
    const ring = correct
    ...
    return (
      <div
        ...
        style={{ width: M_COL_W - 10, height: M_WD_H, fontFamily: FONT }}
      >
        {fixedLabel ? (
          fixedLabel
        ) : wdName ? (
          ...
          <div className="flex items-center gap-0.5">
            ...
            <input
              ...
              onChange={(e) => onChange?.(e.target.value)}
              placeholder="?"
              className={`w-6 border-none text-center text-xs font-bold outline-none ${inputCls}`}
              style={{ fontFamily: FONT }}
            />
          </div>
        )}
...
const MSegCol = memo(
  ({
    ...
    seg: MSegment
    answer: MSegAnswer
    onAnswer: (p: Partial<MSegAnswer>) => void
    daysOk: boolean
    wdRole: MWdRole
    wdFixed?: string
    wdValue?: string
    wdOnChange?: (v: string) => void
    wdCorrect?: boolean
    wdWrong?: boolean
  }) => (
    <div className="relative shrink-0" style={{ width: M_COL_W, height: M_CELL_H }}>
      <div
        ...
        style={{ top: 0, height: M_LBL_H }}
      >
        <div
          className={`rounded-xl px-2.5 py-0.5 text-[11px] font-semibold whitespace-nowrap select-none ${seg.isEndpoint ? 'bg-slate-200 text-slate-700' : 'bg-slate-100 text-slate-400'}`}
          style={{ fontFamily: FONT, lineHeight: `${M_LBL_H - 4}px` }}
        >
          {seg.label}
        </div>
      </div>
      <div
        className="absolute inset-x-0 flex items-center justify-center"
        style={{ top: M_LBL_H + 10, height: M_PILL_H }}
      >
        <MDaysPill
          value={answer.days}
          onChange={(v) => onAnswer({ days: v })}
      ...
      <div
        className="absolute inset-x-0 flex items-center justify-center"
        style={{ top: M_WD_Y, height: M_WD_H }}
      >
        <MWdPill
          role={wdRole}
          fixedLabel={wdFixed}
...
const MGapConnector = memo(
  ({ answer, onAnswer }: { answer: MSegAnswer; onAnswer: (p: Partial<MSegAnswer>) => void }) => {
    ...
    return (
      <div className="relative shrink-0" style={{ width: M_GAP_W, height: M_CELL_H }}>
        <div
          className="absolute inset-x-0 flex items-center justify-center gap-0.5"
          style={{
            top: M_WD_Y - 12,
            height: 20,
            color: '#f43f5e',
        ...
        >
          <span>+</span>
          <MNum value={answer.r} onChange={(v) => onAnswer({ r: v })} width={26} cls={roseNum} />
        </div>
        <div
          className="absolute inset-x-0 flex items-center"
          style={{ top: M_WD_MID_Y - 1, height: 2 }}
        >
          <div className="flex-1 bg-slate-300" style={{ height: 2 }} />
          <svg width="9" height="9" viewBox="0 0 9 9" className="-ml-px shrink-0">
            <path
              d="M1 1.5L7.5 4.5L1 7.5"
              fill="none"
              stroke="#94a3b8"
              strokeWidth="2"
              strokeLinecap="round"
...
const MonthTail: React.FC<MonthTailProps> = ({ startDate, endDate, startWeekday }) => {
  ...
  const n = segs.length
  const [answers, setAnswers] = useState<MSegAnswer[]>(() =>
    segs.map(() => ({ days: '', q: '', r: '' })),
  )
  const [wdInputs, setWdInputs] = useState<string[]>(() => Array(n).fill(''))

  const _patchAnswer = useCallback(
    (i: number, p: Partial<MSegAnswer>) =>
      setAnswers((prev) => {
        const nx = [...prev]
        nx[i] = { ...nx[i], ...p }
        return nx
  ...
  const hints = useMemo(() => {
    ...
    wdInputs.forEach((v, i) => {
      if (v !== '' && !wdValid[i]) h.push(`节点 ${i + 1} 星期应为 ${MONTH_WD[correctWds[i + 1]]}`)
    })
    return h
  ...
  return (
    <div className="flex w-full flex-col items-center gap-3">
      <div className="text-xs font-medium text-slate-400" style={{ fontFamily: FONT }}>
        ↓ 跨月计算（{startDate} → {endDate}）
      </div>
      <div className="w-full overflow-x-auto rounded-2xl border border-slate-100 bg-white shadow">
        <div
          className="flex items-stretch px-4 py-5"
      ...
      <div
        className={[
          'flex w-full items-center gap-3 rounded-xl px-4 py-2.5 text-sm font-semibold transition-all duration-300',
          allOk
      ...
      >
        <span className="flex-1">
          {allOk
            ? `✓ 跨月计算正确！结束日期是 ${MONTH_WD[correctWds[n]]}`
            : hints.length
        ...
        <button
          onClick={reset}
          className="shrink-0 cursor-pointer rounded-full bg-white/50 px-2.5 py-0.5 text-[11px] font-medium transition-all hover:bg-white/80 active:scale-95"
...
// ── Main component ─────────────────────────────────────────────────────────
interface YearRowAnswer {
  days: string
  plus: string
}

const Year: React.FC<YearlyWeekdayFlowChartProps> = ({
  startDate,
  endDate,
  startWeekday,
  endWeekday,
  ...
  const [answers, setAnswers] = useState<YearRowAnswer[]>(() =>
    segs.map(() => ({ days: '', plus: '' })),
  )
  ...
  const patchAnswer = useCallback(
    (i: number, p: Partial<YearRowAnswer>) =>
      setAnswers((prev) => {
        const nx = [...prev]
        nx[i] = { ...nx[i], ...p }
        return nx
  ...
  if (n === 0)
    return (
      <div className="py-8 text-center text-sm text-slate-400" style={{ fontFamily: FONT }}>
        日期范围无效（需跨年）
      </div>
    )
  ...
  return (
    <div className="flex w-full flex-col items-center gap-4">
      {/* Year chart */}
      <div className="w-full overflow-x-auto rounded-2xl border border-slate-100 bg-white shadow-lg">
        <div className="flex flex-col px-8 py-6" style={{ width: 'max-content', margin: '0 auto' }}>
          {rows}
        </div>
      </div>
