...
import Lesson45Provider, { useLesson45 } from '@/components/math/lesson45/Lesson45Provider'
import AppHeader from '@/components/math/lesson45/AppHeader'
import Sidebar from '@/components/math/lesson45/Sidebar'
import BottomNav from '@/components/math/lesson45/BottomNav'
...
function getNextHref(pathname: string): string | undefined {
  ...
  return `/math/ny/45/${section}/${index + 1}`
}

function InnerLayout({ children }: { children: React.ReactNode }) {
  const { toast, setToast, showCongrats, setShowCongrats } = useLesson45()
  const pathname = usePathname()
  const nextHref = getNextHref(pathname)

  return (
    <div
      className="flex min-h-screen flex-col bg-[#fff7ed] text-[15px] text-text-primary"
...
export default function Lesson45Layout({ children }: { children: React.ReactNode }) {
  return (
    <Lesson45Provider>
