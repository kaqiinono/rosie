...
import Lesson13Provider, { useLesson13 } from '@rosie/math/components/lesson13/Lesson13Provider'
import AppHeader from '@rosie/math/components/lesson13/AppHeader'
import Sidebar from '@rosie/math/components/lesson13/Sidebar'
import BottomNav from '@rosie/math/components/lesson13/BottomNav'
...
function getNextHref(pathname: string): string | undefined {
  ...
  return `/math/ny/13/${section}/${index + 1}`
}

function InnerLayout({ children }: { children: React.ReactNode }) {
  const { toast, setToast, showCongrats, setShowCongrats } = useLesson13()
  const pathname = usePathname()
  const nextHref = getNextHref(pathname)

  return (
    <div
      className="flex min-h-screen flex-col bg-green-50 text-[15px] text-text-primary"
...
export default function Lesson13Layout({ children }: { children: React.ReactNode }) {
  return (
    <Lesson13Provider>
