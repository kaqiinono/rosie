...
import { TAG_STYLE, TYPE_TIP } from '@rosie/math/utils/lesson49-data'
...
export default function ProblemDetail({
  ...
  const { solveCount, handleSolve, addWrong } = useLesson49()
  ...
  function checkAnswer() {
    ...
    if (v === problem.finalAns) {
      ...
      setFeedback({ text: `❌ 不对哦，再想想？提示：可以用凑整、去括号、基准数等方法巧算。`, ok: false })
      addWrong(problem.id)
    }
  }
  ...
  const answerDom = (
    <>
      ...
      <div className="mb-3 rounded-lg border border-dashed border-border-light bg-[#f9fafb] p-3.5">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
          ...
          <button onClick={checkAnswer}
            className="cursor-pointer rounded-full bg-indigo-600 px-4 py-2 text-[13px] font-semibold text-white shadow-[0_3px_10px_rgba(67,56,202,0.3)] transition-all active:translate-y-px">
            检查答案
          </button>
        </div>
        {feedback && (
      ...
      {tipText && (
        <div className="mb-3 rounded-lg bg-indigo-50 px-3 py-2.5 text-xs leading-relaxed text-indigo-800">
          💡 <strong>巧算口诀：</strong>
          {tipText}
        </div>
      )}
